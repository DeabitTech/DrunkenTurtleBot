...
class Status:
    ...
    def __init__(self, client_name, password, market_data, run=True, allow_order_placing=True, *args):
        ...
        if self.run:
            ...
            self.serverTime = int(int(self.get_server_time(from_server=True)["serverTime"]) / 1000)
            sleep(self.wait)
            ...
            if market_data["needsRestart"] == True:
                self.openOrders = self.cancelAllOpenOrders()
                market_data["needsRestart"] = False
            ...
            self.market_info = self.client.get_symbol_info(self.marketSymbol)
            sleep(self.wait)

            logMessage = f'MarketData:\t{self.marketData}\nFree Balances:\t{self.freeBalances}\nTotalBalances:\t{self.totalBalances}'
            log.raw_log_append(logMessage)
            ...
            for task in self.tasksList:
                if self.allow_order_placing:
                    try:
                        if task["taskType"] == "nullTask":
                            pass
                        elif task["taskType"] == "createOrder":
                            self.client.create_order(**self.marketSymbolDict,  type='limit', timeInForce='GTC', side=task["side"], quantity=task["quantity"], price=task["price"])
                        ...
                        raise NewTaskError(err.__repr__())
                    except Exception as err:
                        raise NewTaskError('Task not found or incorrectly formatted. Skipping this step...  -  ' + err.__repr__())
    ...
    def get_server_time(self, from_server=False):
        if from_server:
    ...
    def get_open_orders(self, fromServer=False):
        if fromServer:
            self.openOrders = self.client.get_open_orders(**self.marketSymbolDict)
        return self.openOrders

    def cancelAllOpenOrders(self):
        try:
            self.client.cancel_all_open_orders(**self.marketSymbolDict)
        except:
            oos = self.client.get_open_orders(**self.marketSymbolDict)
            for oo in oos:
                self.client.cancel_order(**self.marketSymbolDict, orderId=oo["orderId"])
                sleep(self.wait)
        self.openOrders = self.client.get_open_orders(**self.marketSymbolDict)

    def getLastFilledOrder(self):
        allOrders = self.client.get_all_orders(**self.marketSymbolDict)
        lastOrder = {}
        if len(allOrders) != 0:
    ...
    def get_total_bnb_balance(self):
        for a in self.accountInfo["balances"]:
            if a["asset"] == BNB_SYMBOL:
    ...
    def get_list_of_new_orders_log_csv_variables(self, task):
        ...
        list.append(self.get_total_bnb_balance())
        return list
